...
-c, --csv=CSV            CSV input file path (for batch mode).
...
if(!is.na(rad)) {
       result <- as.data.frame(dfci_final %>%
         ...
         dplyr::select(collection_lat, collection_lon, sample_time, measurement_lat, measurement_lon, ci, chl, dist_m, radius_limit_m, ci_mean, chl_mean, model_image) %>%
...
if (is.null(arguments$csv)) {
  ...
  rad <- ifelse(!is.null(arguments$r), arguments$r, NA)
  output_file <- arguments$o  #output path
  ...
  if (!is.null(rad) & !is.na(rad)) {
    result <- as.data.frame(dfci_final %>%
                              ...
                              dplyr::select(
                                collection_lat, collection_lon, sample_time,
