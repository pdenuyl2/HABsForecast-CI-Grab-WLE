...
datestring1 <- substr(datestring, 1, 7)
  ...
  fntxt <- paste0(imagedir, format(imageDate, "M%Y%j%H%M00.txt") )
...
#    points(dfrc$lon, dfrc$lat, pch=15, cex=0.5)
  ...
  ptsmat <- as.matrix(dfrc[,c("lon","lat","chl","temp")])
  ptsmat1 <- sprintf(fmt="%12.6f %12.6f %10.6f %10.6f", ptsmat[,1], ptsmat[,2] , ptsmat[,3], ptsmat[,4] )
  write.table(ptsmat1, file = fntxt, append = FALSE, row.names=FALSE, col.names=FALSE, quote=FALSE)
