...
Usage:
  extract_habtracker_ci_v2.R --lat latitude --lon longitude -d date [options]
...
Options:
  ...
  -t, --time=TIME          Time of sample collection in Eastern Daylight Time (EDT) (format: hh:mm). DEFAULT = 12:00.
...
source("/geomicro/data2/pdenuyl2/CIGLR_bioinformatics/2024/MODIS_acquisition/code/hab_tracker_2017_code_share/hab_tracker/proj_functions.R")
...
##Designate proj_fuctions - source: hab_tracker_2017_code_share/hab_tracker/proj_functions.R# use the GLCFS Lambert conformal projection for plots
#library(sp)

##load packages 
library(geosphere)
#library(terra) 
#install.packages("packages/sp_2.2-0.tar.gz", repos=NULL, type="source")
#library(sp)
...
library(raster)
#library(PBSmapping)
#ignore: library(ncdf4)
#ignore: library(fields)

#Define features
#Coordinates - Format: decimal
lat <- as.numeric(arguments$lat)   #latitude
lon <- as.numeric(arguments$lon) #longitude
...
ifelse(!is.null(arguments$`--rad`),
       #print all values within radial distance assigned to -r, --rad
       #remove any values further than assigned radial distance
       result <- as.data.frame(dfci_final %>%
         filter(dist_m <= as.numeric(arguments$`--rad`)) %>%
