...
Options:
  ...
  -i, --csv=CSV            CSV containing lat/lon/date/time/rad inputs for multiple samples, by row.
...
arguments <- docopt(doc, args = c(" -l 41.8077 -L -83.3435 -d 2021-08-19 -t 14:00 -c "/geomicro/data2/pdenuyl2/CIGLR_bioinformatics/2025/MODIS_acquisition/test.csv""))
#print(arguments)
...
if (!is.null(arguments$csv)) {   # CSV mode
  ...
  # Prepare overall results for bulk import # CSV model
...
#updated function: 8Aug2025 PD
process_sample <- function(lat, lon, collection_date, collection_time = "12:00", radial = NULL) {
if (!is.null(arguments$`--rad`)) {
  return(as.data.frame(dfci_final %>%
...
#updated function: 8Aug2025 PD
res <- process_sample(lat, lon, collection_date, collection_time, arguments$`--rad`)
